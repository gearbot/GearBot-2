...
pub fn on_channel_delete(channel: TwilightChannel, context: &Arc<BotContext>) {
    if let Some(old) = cache_channel_delete(channel, context) {

    }
}

pub fn cache_channel_delete(channel: TwilightChannel, context: &Arc<BotContext>)-> Option<Arc<Channel>> {
    if let TwilightChannel::Guild(guild_channel) = channel {
        if let Some(guild_id) = guild_channel.guild_id() {
            return context.cache.remove_channel(&guild_id, &guild_channel.id());
        } else {
