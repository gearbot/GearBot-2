...
pub async fn async_followup(command: Box<ApplicationCommand>, state: &Arc<State>) -> Result<(), CommandError> {
    ...
    match component.as_str() {
        "guild_config" => {
            let info = state.datastore.get_or_create_guild_info(&command.guild_id.unwrap()).await?;
            let bytes = serde_json::to_vec_pretty(&info.config)?;
            state.discord_client.create_followup_message(&command.token)
                .unwrap()
                .attach(
        ...
        _ => {
            state.kafka_sender.send(
                "gearbot_cluster_0",
                &Message::new_interaction(command.token,
