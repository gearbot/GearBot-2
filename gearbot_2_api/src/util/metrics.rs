...
impl Metrics {
    pub fn new() -> Self {
        ...
        let http_requests_total = IntCounterVec::new(Opts::new("http_requests_total", "Total handled http requests per method, route and status"), &["method", "path", "status"]).unwrap();
        registry.register(Box::new(http_requests_total.clone())).unwrap();

        let http_requests_duration = HistogramVec::new(
            HistogramOpts::new("http_requests_duration", "Duration for the recent http requests per method, route and status"),
            &["method", "path", "status"]).unwrap();
        registry.register(Box::new(http_requests_duration.clone())).unwrap();

        let command_totals = IntCounterVec::new(Opts::new("command_totals", "Total handled command invocations by result"), &["command_name", "result"]).unwrap();
        ...
        let command_followups_total = IntCounterVec::new(Opts::new("command_followups_total", "Total async command followups invocations by result"), &["command_name", "result"]).unwrap();
        registry.register(Box::new(command_followups_total.clone())).unwrap();

        let command_followups_duration = HistogramVec::new(
            HistogramOpts::new("command_followups_duration", "Duration of async command followup handler invocations")
                .buckets(vec![0.5, 1.0, 2.5, 5.0, 10.0, 15.0, 30.0, 60.0, 120.0, 300.0, 600.0, 900.0]),
            &["command", "result"])
            .unwrap();
